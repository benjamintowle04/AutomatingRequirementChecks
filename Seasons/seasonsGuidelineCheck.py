...
wb_requirementsCheck = load_workbook(
    r"C:/DSSO/AutomatingRequirementChecks/Seasons/Shifts.xlsx"
)
...
class Requirement:
    def __init__(
        self, requirement_name, requirement_type, excused, exception, min_shifts
    ):
...
def main():
    ...
    seasons_regular_guideline = getFacilityGuidelineRegular(
        ws_requirements, ws_times_in_need
    )
    seasons_supervisor_guideline = getFacilityGuidelineSupervisor(
            ...
            if not (
                ...
                if req.exception:
                    if excuseRequirement(employee, req.exception, req):
                        employee.notes = req.exception.exception_note
...
def countMetRequirements(employee, requirement_type):
    ...
    if isinstance(requirement_type, DayRequirement):
        ...
        for shift in employee.schedule:
            ...
            if shift.end_time >= convert_to_time(
                "12:00:00 AM"
            ) and shift.end_time <= convert_to_time("3:00:00 AM"):
